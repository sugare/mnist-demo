...
def build_flags():
    ...
    parameters = [
        ...
        ['--data_dir', str, '/clever/admin/mnist-cnn/mnist-data', 'Directory for storing mnist data'],
        ['--log_dir', str, '/clever/admin/logs', 'Directory for train log'],
        ['--model_dir', str, '/clever/admin/mnist_model', 'Directory for storing mnist model'],
        ['--model_version', int, 1, 'Version of mnist model'],
        ['--num_gpus', int, 0, 'Total number of gpus for each machine'],
...
def init_tf_config():
    ...
    if FLAGS.job_name == 'ps':
        ...
        return
    os.system('nvidia-smi -q -d Memory |grep -A4 GPU|grep Free >/tmp/gpu_info')
    memory_gpu=[int(x.split()[2]) for x in open('/tmp/gpu_info','r').readlines()]
    
    for i in range(len(memory_gpu)):
        if memory_gpu[i] > 10000:
